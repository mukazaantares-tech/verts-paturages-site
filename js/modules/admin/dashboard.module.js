...
=============================== */
  ...
  async renderStats() {
    try {
      const [
        ...
        supabaseClient.from("church_members").select("*"),
        supabaseClient.from("adhesion_requests").select("*"),
        supabaseClient.from("departements").select("*"),
        supabaseClient.from("instructions").select("*"),
        supabaseClient.from("admins").select("*"),
      ]);
      document.getElementById("statMembres").textContent = membres?.length || 0;
  ...
  async renderChart() {
    try {
      let canvas = document.getElementById("globalStatsChart");
      if (!canvas) return;
      if (chartInstance) {
      ...
      const membresCount = membres?.length || 0;
      const adhesionsCount = adhesions?.length || 0;
